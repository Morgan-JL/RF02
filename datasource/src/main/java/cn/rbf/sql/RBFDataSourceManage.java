...
import cn.rbf.annotation.NonNull;
 ...
 */
public class RBFDataSourceManage {

    /*
        数据源管理器
     */

    private RBFDataSourceManage(){}
    ...
    private static Map<String, RBFDataSource> dbMap=new HashMap<>();
    /** 数据源管理器支持的连接池类型 [pool-type => DataSourceType]*/
    private static Map<String,Class<? extends RBFDataSource>> poolTypeMap=new HashMap<>();
     ...
     */
    public static RBFDataSource getDataSource(String dbname){
        init();
        RBFDataSource luckyDataSource = dbMap.get(dbname);
     ...
     * @return 返回所有的数据源
     */
    public static List<RBFDataSource> getAllDataSource(){
        init();
        List<RBFDataSource> luckyDataSources=new ArrayList<>(dbMap.size());
        for(Map.Entry<String, RBFDataSource> dataSourceEntry:dbMap.entrySet()){
    ...
    /**
     * 注册一个新的连接池类型<br/>
     * 这里注册的连接池类型将会以 pool-type ==> RBFDataSource.class的形式
     * 存放在{@link RBFDataSourceManage#poolTypeMap}中，当需要从配置文件解析
     * 生成数据源时管理器就会根据配置信息中的pool-type来找到对应的连接池类型，并使用
     * 该类型来实例化这个链接池<br/>
     * 注：<br/>
     * 1.数据源管理器会将第一个注册进来的连接池作为默认的连接池类型<br/>
     * 2.这个连接池类型必须为{@link RBFDataSource}的子类
     * @param poolType 连接池的类型
     */
    public static void registerPool(@NonNull Class<? extends RBFDataSource> poolType){
        Assert.notNull(poolType,"无法注册连接池信息: 因为poolType为NULL！");
        RBFDataSource luckyDataSource = ClassUtils.newObject(poolType);
     ...
     */
    public static void addLuckyDataSource(RBFDataSource luckyDataSource){
    ...
    public static void confLuckyDataSource(){
        ...
        for(Map.Entry<String,Object> datasource:datasources.entrySet()){
            ...
            if(!(datasourceValue instanceof Map)){
                RBFDataSource luckyDataSource = createLuckyDataSourceByConf(DEFAULT_DBNAME,datasources);
            ...
            RBFDataSource luckyDataSource = createLuckyDataSourceByConf(datasource.getKey(),dataInfo);
    ...
    private static RBFDataSource createLuckyDataSourceByConf(String dbname,Map<String,Object> confMap){
        //获取数据源配置中指定要使用的连接池类型
        ...
        RBFDataSource luckyDataSource = ClassUtils.newObject(poolTypeMap.get(poolType));
        //根据配置信息初始化这个数据源
    ...
    public static void destroy(){
        for(Map.Entry<String, RBFDataSource> entry: dbMap.entrySet()){
